...
- [ ] CI checks out `intentproof-spec` at the **declared commit SHA** (see consumer workflows / `scripts/read-consumer-spec-commit.sh`), not an unconstrained `main` tip that may bypass the pin contract.
...
- [ ] CI runs a hardening audit script (for example `intentproof-spec/scripts/check-consumer-hardening.sh`) to enforce these controls mechanically.
...
## 7) Consumer parity spot checks

- [ ] At least one CI/test path compares canonicalized emitted events across language implementations for the same scenario set.

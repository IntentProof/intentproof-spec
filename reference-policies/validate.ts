...
export type ValidateReferencePoliciesOptions = {
  root?: string;
  repoRoot?: string;
};

export type ValidateReferencePoliciesResult = {
  ok: boolean;
  messages: string[];
  packCount: number;
};
...
export function validateReferencePolicies(
  options: ValidateReferencePoliciesOptions = {},
): ValidateReferencePoliciesResult {
  const root = options.root ?? __dirname;
  const repoRoot = options.repoRoot ?? path.join(root, '..');
  ...
  if (!policySchema || !flowSchema || !runSchema) {
    return {
      ok: false,
      messages: ['required schemas failed to load'],
      packCount: 0,
    };
  }
  ...
  if (errorCount === 0) {
    messages.push(`Reference policy validation passed (${referenceIDs.size} pack(s)).`);
  }

  return {
    ok: errorCount === 0,
    messages,
    packCount: referenceIDs.size,
  };
}

export function runValidateReferencePoliciesCli(): number {
...
if (require.main === module) {
  process.exit(runValidateReferencePoliciesCli());
}
/* v8 ignore stop */

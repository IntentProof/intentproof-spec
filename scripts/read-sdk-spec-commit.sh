...
if [[ $# -ne 1 ]]; then
  echo "usage: read-sdk-spec-commit.sh /path/to/sdk-repo" >&2
  exit 2
fi
...
NODE
  ...
  if [[ "$ec" -eq 2 ]]; then
    echo "read-sdk-spec-commit: root and packages/sdk package.json must declare matching intentproofSpecCommit" >&2
    exit 2
  fi
  if [[ "$ec" -eq 3 ]]; then
    echo "read-sdk-spec-commit: intentproofSpecCommit must be 40-char lowercase hex" >&2
    exit 2
  fi
  if [[ "$ec" -ne 0 ]]; then
    echo "read-sdk-spec-commit: failed reading Node package.json pins (exit ${ec})" >&2
    exit "$ec"
  fi
  exit 0
fi
...
if [[ -f "${sdk_root}/build.gradle.kts" ]]; then
...
echo "read-sdk-spec-commit: could not detect SDK type under ${sdk_root}" >&2
exit 2

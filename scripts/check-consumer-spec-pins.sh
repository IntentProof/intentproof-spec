#!/usr/bin/env bash
# Canonical consumer-repository pin contract: declared spec version + immutable spec commit must match the spec checkout.
# Usage:
#   bash scripts/check-consumer-spec-pins.sh CONSUMER_ROOT SPEC_ROOT
set -euo pipefail

if [[ $# -ne 2 ]]; then
  echo "usage: check-consumer-spec-pins.sh CONSUMER_ROOT SPEC_ROOT" >&2
  exit 2
fi
...
if [[ ! -f "${spec_root}/spec.json" ]]; then
  echo "check-consumer-spec-pins: not a spec checkout (missing spec.json): ${spec_root}" >&2
  exit 2
fi

if ! git -C "$spec_root" rev-parse HEAD >/dev/null 2>&1; then
  echo "check-consumer-spec-pins: spec_root is not a git checkout: ${spec_root}" >&2
  exit 2
fi
...
if [[ ! "$head_sha" =~ ^[a-f0-9]{40}$ ]]; then
  echo "check-consumer-spec-pins: unexpected git rev-parse output: ${head_sha}" >&2
  exit 2
fi

fail_pin() {
  echo "check-consumer-spec-pins: $*" >&2
  exit 1
}
...
NODE
  ...
  _pin_tmp="$(mktemp)"
  python3 - "${consumer_root}/pyproject.toml" <<'PY' >"${_pin_tmp}"
import tomllib
import sys
...
PY
  ...
  if [[ ! -f "$props" ]]; then
    fail_pin "gradle.properties missing for Java consumer repository layout"
  fi
  _pin_tmp="$(mktemp)"
...
PY
  ...
  echo "check-consumer-spec-pins: could not detect consumer repository layout under ${consumer_root} (expected Node workspace, pyproject.toml, or Gradle Kotlin DSL)" >&2
  exit 2
fi

echo "Spec pins OK (version=${spec_version} commit=${head_sha})"
